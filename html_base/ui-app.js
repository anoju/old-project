...
$(document).ready(function(){
	/* 아이폰X 관련 메타태그 : sbp.js 참고 */
	$("meta[name=viewport]").attr('content','width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0, viewport-fit=cover');

	//#content-section 중복 노출 시 하위 #content-section의 id 속성 제거
	if($('#content-section #content-section').length > 0)$('#content-section #content-section').removeAttr('id');

	//디바이스 앱에서 화면사이즈 (메인페이지에서) 확인용
	//if($('.main-section').length > 0)$('.main-section').prepend('<h1>넓이: '+$(window).width()+', 높이: '+$(window).height()+'</h1>');
...
var preLoading = function(){
	var isAppPreLoading = sessionStorage.getItem('isAppPreLoading'),
		$class = 'pre-loading',
		$imgarry = [
	...
	if(isAppPreLoading != 'true'){
		...
		sessionStorage.setItem('isAppPreLoading',true);
		$('body').append($html);
	}
};
...
var commonUi = function(){
	...
	/* S: 스크립트로 제어하나 이슈로 인해 수동으로 클래스 재입력 처리 */
	//.input-tbl, .input-add-txt
	$('.input-tbl').each(function(){
		$(this).addClass("col-" + $(this).children().children().size());
	});
	$('[class*="input-add-txt"]').each(function(){
		var $txt = $(this).children('.txt').text(),
			$length = String($txt).length;
		if($length > 1)$(this).addClass("word-" + String($txt).length);
	});

	//step
	$('.step-02, .tabmenu-01').each(function(){
		var $active = $(this).find('.active');
		if($active.find('.blind').length < 1)$active.prepend('<span class="blind">현재위치</span>');
	});
	/* E: 스크립트로 제어하나 이슈로 인해 수동으로 클래스 재입력 처리 */

	//input:file
	$(document).on('focus','.input-file > span:first-child input',function(){
		$(this).closest('.input-file').find('.btn-file input').trigger('click');
	});
	$(document).on('click','.input-file .btn-file .button',function(e){
		e.preventDefault();
		$(this).closest('.input-file').find('.btn-file input').trigger('click');
	});
	$(document).on('change','.input-file .btn-file input',function(){
		$(this).closest('.input-file').find('> span:first-child input').val($(this).val());
	});

	//input-del : input 입력 삭제버튼
	$('.input-del').each(function(){
		var $parent = $(this).parent();
		if(!$parent.hasClass('input-box'))$parent.addClass('input-box');
		if($(this).siblings('.btn-search').size() > 0)$parent.addClass('type2');
	});
	...
	//투자성향설문
	$('.icon-radio-list [type=radio]').change(function(){
		var $next = $(this).closest('ul').closest('li').next(),
			$wrap = $('html,body');
		if($next.length > 0){
			var $top = $next.offset().top;
			if($(this).closest('.layer.full').length > 0){
				$wrap = $(this).closest('.layer-cont');
				$top = $next.position().top;
			}
			$wrap.delay(300).animate({'scrollTop':($top-70)},500);
	...
	//키패드 활성화 시 하단고정버튼 숨기기(ios12 이상 제외)
	//ios12버전 키패드가림 버그 수정(네이트브 앱을 수정할수 있는 여건이 아니라고 함)
	...
	$(document).on('focusin','input, select, textarea',function(){
		...
		$this.addClass('focus');
		if(isMobile.Android() && $this.is('select')) return;		//안드로이드에서 select 제외
		clearTimeout(isInputFocusTime);
		if(isInputFocus == false && !$this.prop('readonly') && $type != 'radio' && $type != 'checkbox'){
			$('body').addClass('inputFocus').css('min-height',$winHeight);
			var $wrap = $('html, body');
			if($('html').hasClass('ios12') && $this.closest('.layer.full').length == 0){
				setTimeout(function(){
					var $wrapSclTop = $wrap.scrollTop();
					//console.log($wrapSclTop, $winHeight, $top, $wrapSclTop + $winHeight - 320);
					if(($wrapSclTop + $winHeight - 380) < $top){
						$wrap.stop().animate({'scrollTop':$top-80},500);
					}
				},100);
			}
			setTimeout(function(){
		...
		if($('body').hasClass('inputFocus')){
			isInputFocusTime = setTimeout(function(){
				inpFocusReset();
			},10);
		}
	});
	var inpFocusReset = function(){
		isInputFocus = false;
		$('body').removeClass('inputFocus').removeAttr('style');
		if($bodyStyle)$('body').attr('style',$bodyStyle);
	};

	//resizeEnd 이벤트
	$(window).resizeEnd(function(){
		//키패드 내릴때 리셋(ios12 이상 제외)
		var $height = $(this).height();
		if(isMobile.iPhoneVersion() >= 12) return;
		if(isInputFocus == true && $height >= $winHeight){
			inpFocusReset();
		}
...
var etcUi = function(){
	...
	//기간선택 시 input[type=date] 보이게
	$(document).on('click','.input-date-search .btn-wrap .btn',function(){
		var $txt = $(this).text(),
			$wrap = $(this).closest('.input-date-search');
		if($txt == '기간선택'){
			$wrap.addClass('on');
		}else{
			$wrap.removeClass('on');
...
var typingText = function(tar){
	...
	$this.css('height',$this.height()).html('<span class="tp-txt"></span>');
...
var toggleUI = function(btn,isTxtChange){
	$(document).on('click',btn,function(){
		...
		if($(this).hasClass('on')){
			...
			if(isList){
				$($target).closest('li').siblings().find('.btn-toggle-01').removeClass('on');
				$($target).closest('li').siblings().find('.toggle-cont').not('.no-toggle').stop().slideUp($toggleSpeed);
			}
...
var toggleScroll = function(tar){
	...
	if($('.btn-wrap.fixed').length > 0)$gap += $('.btn-wrap.fixed').outerHeight();
...
var btnUi = function(){
	...
	$(document).on('click','a',function(e){
		var $href = $(this).attr('href');
		if($href == '#' || $href == '#none' || $(this).hasClass('img-expand'))e.preventDefault();
	});

	//버튼 클릭 효과
	$(document).on('click', '.btn, .btn-link-01, .main-box .link, .list-data-02 .btn-toggle-01, .list-view-01 .navi-link', function(e){
		var $btnEl = $(this),
			$delay = 650,
			$href = $btnEl.attr('href');
		if($href == '#' || $href == '#none')e.preventDefault();

		//if(!$btnEl.hasClass('disabled') && $btnEl.closest('.btn-wrap').size() > 0){
	...
	//가이드 html 폴딩
	$(document).on('click', '.guide-tit2', function(e){
		var $next = $(this).next();
		if($next.hasClass('guide-inner')){
			$(this).toggleClass('on');
			$next.toggle();
		}
	});
	...
	//포트폴리오 설정
	$(document).on('click','.list-setting .btn-setting',function(e){
		e.preventDefault();
		var $this = $(this),
	...
	$(document).on('click','.ui-img-expand',function(e){
		...
		var $layer = '<div class="layer-wrap img-layer"><div class="layer"><div class="img"></div><a href="#" class="btn-close">닫기</a></div></div>';
		$('body').append($layer);
		$('.img-layer .img').html($img);
	});
	$(document).on('click','.img-layer .btn-close',function(e){
		e.preventDefault();
		$('.img-layer').remove();
	});
};
...
var scrollShadowUi = function(){
	...
	//풀팝업
	$('.layer.full .layer-cont').on('scroll',function(e){
		var $scrollTop = $(this).scrollTop(),
			$closest = $(this).closest('.layer'),
			$head = $closest.find('.layer-head');
		if($closest.hasClass('no-head') || $head.length == 0){
			$head = $closest;
		}
		if($scrollTop > 10){
...
var tooltipUI = function(){
	//열기
	$(document).on('click','.btn-tip-wrap button.btn-tip, .btn-tip-wrap button.ico-text',function(e){
		e.preventDefault();
		var $this = $(this),
			$box = $this.closest('.btn-tip-wrap').find('.tip-box');

		if($box.hasClass('on')){
			$box.removeClass('on');
			//$box.closest('.btn-tip-wrap').removeClass('on');
		}else{
			$('.tip-box').removeClass('on').removeAttr('style');
			$box.addClass('on');
			//$box.closest('.btn-tip-wrap').addClass('on');
		}
	...
	if($('.tip-box').length > 0){
		$(document).on(clickType,function(){
			$('.btn-tip-wrap .tip-box').removeClass('on').removeAttr('style');
		}).on(clickType,'.btn-tip-wrap',function(e) {
			e.stopPropagation();
		});
	}
};
...
var alertText = function(tar,text,val){
	...
	if($error.length > 0){
		...
		var $html = '<div id="'+$ariaId+'" class="alert-txt error-txt';
