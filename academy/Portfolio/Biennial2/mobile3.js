...
$(function(){
	...
	if ($("#bigImg_wrap").length){
		mainR = new roll($("#bigImg_wrap"));
		mainR.speed = 400;
		mainR.autoTime = 5000;
		mainR.paging = true;
		mainR.init($("#bigImg_wrap"));
	}
...
var roll = function(obj){
	...
	this.init = function(obj){
		...
		if (root.paging && root.mNum > 1){
			...
			$(root.obj).find(".rollimg a").each(function(idx){
				if (idx == 0){
					$(this).addClass("on");
				}
	...
	this.rolAc = function(n){
		...
		if (this.paging){
			$(this.obj).find(".rollimg a").removeClass("on");
			$(this.obj).find(".rollimg a").eq(this.sNum).addClass("on");
		}
