...
function backBtnPos(){
	...
	console.log(chkClass,idx2);
	if ( chkClass == 'big' ) {
		if( idx2 == 1){
			console.log('test');
			$( '.back' , idx2Box ).css({ 'left' : '381px'});
			$( '.over_text' , idx2Box ).css({ 'bottom' : '84px' });
		}
		else if( idx2 == 2){
			$( '.back' , idx2Box ).css({ 'left' : '732px'});
		}
		else if( idx2 == 3){
			$( '.back' , idx2Box ).css({ 'left' : '971px'});
		}			
		else if( idx2 == 4){
			$( '.back' , idx2Box ).css({ 'left' : '724px' });
		}
		else if( idx2 == 5){
			$( '.back' , idx2Box ).css({ 'left' : '30px'});
		}
	}
	else if ( chkClass == 'small' ) {
		if( idx2 == 1){
			$( '.back' , idx2Box ).css({ 'left' : '704px' });	
			$( '.over_text' , idx2Box ).css({ 'bottom' : '200px' });
		}
		else if( idx2 == 2){
			$( '.back' , idx2Box ).css({ 'left' : '467px' });	
		}
		else if( idx2 == 3){
			$( '.back' , idx2Box ).css({ 'left' : '698px' });	
		}			
		else if( idx2 == 4){
			$( '.back' , idx2Box ).css({ 'left' : '473px' });	
		}
		else if( idx2 == 5){
			$( '.back' , idx2Box ).css({ 'left' : '30px' });	
		}
		else if( idx2 == 6){
			$( '.back' , idx2Box ).css({ 'left' : '240px' });	
		}
	}
}
//test end
...
//	$('.graBox').css({ 'width' : lgIntro.width() + 'px' }) ;
	...
	fixedNavi.hide().css({ 'top' : (scrollH - fixedNavi.height() - parseInt(fixedNavi.css('right')) ) + 'px' }) ; /* 2012-10-29 수정 */	
	fixedNavi.show() ;
...
//	$('body').addClass('scrollOff-y') ;	
	...
	if ( chkMoveNum > -1 ) {
		...
		if (UserAgent.match(/iPhone|iPad|iPad2|iPod|Android|Windows CE|BlackBerry|Symbian|Windows Phone|webOS|Opera Mini|Opera Mobi|POLARIS|IEMobile|lgtelecom|nokia|SonyEricsson|SAMSUNG|Samsung/) != null && $(window).width() >= 768){
			if ( orientation == 0 || orientation == 180 ) {
				guageBox
				.css({ 'top' : $('div.introBox').height() / 2 + 'px' , 'margin-left' : - guageBox.width() / 2 + 'px' ,  'margin-top' : - guageBox.height() / 2 + 'px'  })
				.fadeTo( 1000 , 1.0 ) ;	
			}
		} else {
			guageBox
...
function guageBox(t){
	...
	guage.animate({ 'height' : t + 'px' } , 1 , 'easeInOutQuart' , function(){
		if(t==303){
...
$(function(){	
	...
	for(var i = 0; i<loadedImgs.length; i++){
		//console.log(loadedImgs[i], i);
		if (Browser.isIE) 
		{
			$('#tmp').append('<img src=\"'+loadedImgs[i]+'\" width=0 height=0/>');
		}
		else
		{
			$('#tmp').append('<img src=\"'+loadedImgs[i]+'\" width=0 height=0/>');
	...
	imgs.each(function(){
		$(this).imagesLoaded(function(){
			img_count++;
			nl = Math.floor((img_count/allimg_count)*100);
			guageBox(parseInt((nl/100)*303))
		})
	})
})
...
//			$('.movBox').html('<object width="687" height="391"><param name="movie" value="http://www.youtube.com/v/QRLRp1vGvfA?version=3&amp;hl=ko_KR&amp;rel=0&amp;autoplay=' +autoPlayChk + '&amp;autohide=0"></param><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><param name="wmode"     value="transparent"/></param><embed src="http://www.youtube.com/v/QRLRp1vGvfA?version=3&amp;hl=ko_KR&amp;rel=0&amp;autoplay=' +autoPlayChk + '&amp;autohide=1" type="application/x-shockwave-flash" width="687" height="391" allowscriptaccess="always" allowfullscreen="true" wmode="transparent"></embed></object>') ;
			$('.movBox').html('<object width="687" height="391"><param name="movie" value="http://www.youtube.com/v/q3ap0k72Dxw?version=3&amp;hl=ko_KR&amp;rel=0&amp;autoplay=' +autoPlayChk + '&amp;autohide=0"></param><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><param name="wmode"     value="transparent"/></param><embed src="http://www.youtube.com/v/q3ap0k72Dxw?version=3&amp;hl=ko_KR&amp;rel=0&amp;autoplay=' +autoPlayChk + '&amp;autohide=1" type="application/x-shockwave-flash" width="687" height="391" allowscriptaccess="always" allowfullscreen="true" wmode="transparent"></embed></object>') ;
		} else if ( $('body').attr('class') == 'small' ) {
			$('.movBox').html('<object width="593" height="337"><param name="movie" value="http://www.youtube.com/v/q3ap0k72Dxw?version=3&amp;hl=ko_KR&amp;rel=0&amp;autoplay=' + autoPlayChk + '&amp;autohide=1"></param><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><param name="wmode"     value="transparent"/></param><embed src="http://www.youtube.com/v/q3ap0k72Dxw?version=3&amp;hl=ko_KR&amp;rel=0&amp;autoplay=' +autoPlayChk + '&amp;autohide=1" type="application/x-shockwave-flash" width="593" height="337" allowscriptaccess="always" allowfullscreen="true" wmode="transparent"></embed></object>') ;
		}
		
	} else {
		$('.movBox').html('') ;	/* 2012-10-29 수정 */
	}
...
function overViewHandler () { // 2Depth 디테일을 볼 수 있습니다.	
	...
	//fixedNavi.hide() ;
	var winWidth = $(window).width(); //11.22 일 추가사항 버튼 클릭 시 네베게이션을 화면 밖으로 이동
	fixedNavi.css({right:winWidth+100}); //11.22 일 추가사항 버튼 클릭 시 네베게이션을 화면 밖으로 이동
	var btn = $(this) ;
...
function hideViewHandler (e) { // 2Depth 디테일을 숨깁니다.	
	...
	var naviTop = $(window).height() - fixedNavi.height()-10; //11.22 수정사항 navi top 위치가 정상 동작이 안되는 겅우
	fixedNavi.css({right:10, top:naviTop, display:"block"}); //11.22 2Depth 버튼 클릭 시 화면 밖으로 이동 시킨 미니 네비를 다시 원 위치로 이동
...
function fixedNaviOverHandler (e) { // 미니 네비게이팅 옵션 입니다.
	...
	overBtn.bind( 'click' , function(){
		...
		$('html , body').stop().animate({ 'scrollTop' : posArr[num] } , speedChk , 'easeInOutQuart' , function(){			
			overBtn.show() ;
			/*_PL(rightThumbNavTitle(posArr[num]));*/return false;
		}) ;
	}) ;
}
function rightThumbNavTitle(obj){
	switch(obj){
		case 0:return "ABOVEANDBEYOND";break;
		case 1450:return "PERFECT_VIEWING_ANGLE";break;
		case 2900:return "COLOR_REFINER";break;
		case 4350:return "ABSOLUTE_MOTION_CLARITY";break;
		case 5800:return "PAPER_SLIM";break;
		case 7250:return "4_COLOR_PIXELS";break;
		case 8700:return "INFINITE_CONTRAST";break;
		case 10150:return "NEW_BEGINNING";break;
...
//			$( '.disclaimer' , cont.eq(idx2) ).css({ 'opacity' : '0.0' }).delay(650).animate({ 'opacity' : '1.0' } , 350 , 'easeOutQuart' ) ; /* Back Btn */
...
//			$( '.disclaimer' , cont.eq(idx2) ).css({ 'display' : 'block' }) ; /* Back Btn */
...
//			$( '.disclaimer' , cont.eq(idx2) ).css({ 'opacity' : '0.0' }).delay(650).animate({ 'opacity' : '1.0' } , 350 , 'easeOutQuart' ) ; /* Back Btn */
		...
		if ( idx2 == 1 ) {
			...
			$( '.back' , idx2Box ).css({ 'left' : '704px' , 'opacity' : '0' }).delay(650).css({'left' : '704px'}).animate({ 'opacity' : '1.0' } , 350 , 'easeOutQuart' ) ; /* Back Btn */
//			$( '.disclaimer' , cont.eq(idx2) ).css({ 'opacity' : '0.0' }).delay(650).animate({ 'opacity' : '1.0' } , 350 , 'easeOutQuart' ) ; /* Back Btn */
